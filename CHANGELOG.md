...
### Changed
* `dst_area_or_point` now defaults to `'Point'` (**breaking**, previously `'Area'`) in `stitch_dem` and `merge_and_transform_dem_tiles`, so the default output stays on the native pixel-centered grid of the source tiles and matches the NISAR DEM exactly in georeferencing. Since geoid removal precedes the relabeling, this changes only the output transform/tag, never the height samples.
