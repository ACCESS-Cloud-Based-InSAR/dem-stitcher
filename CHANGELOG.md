...
### Fixed
* `reproject_arr_to_match_profile` and `reproject_arr_to_new_crs` never declared the source nodata to `rasterio.warp.reproject`, so gdal treated *every* source pixel as valid data; they now mask `src_profile['nodata']` by default. gdal enforces that valid output data never collides with the output nodata: when a computed pixel equals `dst_nodata` - which these functions arrange automatically, taking `dst_nodata` from the same profile whose nodata went undeclared - [`AvoidNoData()` in `alg/gdalwarpkernel.cpp`](https://github.com/OSGeo/gdal/blob/v3.12.4/alg/gdalwarpkernel.cpp#L1498) shifts it one representable step (integers decrement, floats move one ulp; `nan` never collides and is immune, which is why the DEM and geoid paths were never affected and the golden datasets, including the ellipsoidal ones asserted at 0.1 mm, are unchanged) and logs `CPLE_AppDefined ... Value 255 in the source dataset has been changed to 254 in the destination dataset to avoid being treated as NoData`. A `uint8` categorical raster with `nodata=255` therefore came back with its entire nodata region as the valid label 254: masks built from the profile miss it, and `merge_arrays_with_geometadata` blends it against the real class labels wherever the grids are not pixel-aligned and `rasterio.merge` resamples. Two independent changes had to land for this to surface, which is what made it so hard to track: gdal policed only *masked* warp paths until 3.11.0 unified the collision avoidance across all paths and added the warning ([gdal#11711](https://github.com/OSGeo/gdal/issues/11711), [gdal PR #11713](https://github.com/OSGeo/gdal/pull/11713), commits [`17a737fb`](https://github.com/OSGeo/gdal/commit/17a737fbd7) and [`2b0b1c5b`](https://github.com/OSGeo/gdal/commit/2b0b1c5bc9), 2025-01-24) - under gdal <= 3.9 an unmasked reproject was never nudged at all - and dem_stitcher 3.0.0 switched the warp destination buffer from `float64`-then-cast to the source dtype, so the one-ulp float nudge that `astype` used to truncate away became a whole-unit integer shift that nothing undid. The 3.10.x boundary is ragged: libgdal 3.10.3's *nearest* fast path demonstrably does not nudge (the regression tests caught this on the former py310 CI environment) while gdal#11711 shows bilinear already nudging in 3.10.1, so the pixi environments now pin `gdal`/`libgdal-core >= 3.11` and the tests assert the artifact unconditionally. `notebooks/analysis_and_comparison/3_Nodata_Remapping_in_Reprojection.ipynb` reproduces the mechanism, the dtype/nodata sweep, the gdal-version/buffer-dtype compatibility matrix, and the downstream merge damage from scratch, with sources.

### Changed
* Minimum supported python raised from 3.10 to 3.11 and the `py310` environment removed from the CI matrix. Python 3.10 reaches end-of-life 2026-10-04, and conda-forge has stopped building the geospatial stack for it: its rasterio builds pair only with `libgdal-core < 3.11` (or `>= 3.11.5`, which needs `proj >= 9.7.1` that no py310 pyproj build permits), so an environment demonstrating the gdal nodata collision behavior above cannot be solved on 3.10.

### Added
* `src_nodata` keyword argument to `reproject_arr_to_match_profile` and `reproject_arr_to_new_crs` for the cases where the source array's nodata is not the profile's. It defaults to `None`, meaning "take it from `src_profile`"; passing a value overrides the profile and emits a `UserWarning`, and `0` is honored as a value rather than swallowed as an omission. `None` deliberately cannot mean "declare nothing" - that is the behavior being fixed - so set `src_profile['nodata'] = None` to reproject a source whose nodata should not be masked (the same escape hatch the `nodata` argument already documents). Note the ecosystem has falsy-zero traps on the *output* side: rasterio's [`_warp.pyx` line 465](https://github.com/rasterio/rasterio/blob/1.5.0/rasterio/_warp.pyx#L465) silently replaces `dst_nodata=0` with `src_nodata`, and this library's own `nodata or src_profile['nodata']` does the same one layer up, so an explicit output nodata of `0` is currently not expressible (demonstrated in the notebook, section 6).
